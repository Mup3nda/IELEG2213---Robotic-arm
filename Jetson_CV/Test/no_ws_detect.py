...
BASE_CORRECTION_FACTOR = 2 # cm (initial correction at close range)
CORRECTION_SCALING = 1.20  # Scaling factor for correction based on distance
...
async def send_data():
    #uri = "ws://192.168.0.178:9000"  # Replace with your WebSocket server URI
        ...
        while True:
            ...
            for detection in detections:
                if detection.ClassID == MOUSE_CLASS_ID:
                    ...
                    corrected_x = round(BASE_CORRECTION_FACTOR + (avg_x * CORRECTION_SCALING ))
                    corrected_y = round(BASE_CORRECTION_FACTOR + (avg_y * CORRECTION_SCALING))
                    ...
                    text = f"Coordinates: x={round(corrected_x)} cm, y={round(corrected_y)} cm, z={corrected_z} cm"
